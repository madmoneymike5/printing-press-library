...
func upsertItem(tx *sql.Tx, item *pb.ListItem) error {
	...
	_, err = tx.Exec(
		`INSERT OR REPLACE INTO items
		 (id, list_id, name, quantity, details, category, category_match_id, checked, manual_sort_index, store_ids, photo_ids, prices, category_assignments)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		item.GetIdentifier(),
		item.GetListId(),
		item.GetName(),
...
func upsertRecipe(tx *sql.Tx, recipe *pb.PBRecipe) error {
	_, err := tx.Exec(
		`INSERT OR REPLACE INTO recipes
		 (id, name, paprika_identifier, note, source_name, source_url, rating, prep_time, cook_time, servings, timestamp, creation_timestamp)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		recipe.GetIdentifier(),
		recipe.GetName(),
		recipe.GetPaprikaIdentifier(),
		recipe.GetNote(),
