...
## AnyList source-shape guardrail

For the AnyList CLI, 300 physical lines is the default maximum for one hand-written source file. This is a firm review gate with a reasoned exception path, not a formatting suggestion. The count includes blank and comment lines. Tests are included; generated protobuf output, vendored code, and machine-generated artifacts are excluded.

Any hand-written source file that exceeds 300 lines requires, before acceptance, a written defense naming its single cohesive responsibility, explaining why splitting it would make the design worse, recording the measured line count, and receiving Sarah Taylor's explicit manual approval. Refactors must split by responsibility, not by arbitrary line count. New code must not knowingly create another over-limit file without that defense and approval.
